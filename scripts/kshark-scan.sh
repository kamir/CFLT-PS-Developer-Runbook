...
CMD_FILE="$RUN_DIR/reports/kshark_cmd.txt"
KSHARK_CMD=(
    "$KSHARK_BIN"
    -props "$PROPS_FILE"
    -topic "$TOPICS_DEFAULT"
    --analyze
    -json "$JSON_OUT"
    -timeout "$KSHARK_TIMEOUT"
    -diag="$KSHARK_DIAG"
    -no-ai
    -y
)
{
    echo "# kshark command (generated by scripts/kshark-scan.sh)"
    echo "# Working directory: $TMP_DIR"
    printf '%q ' "${KSHARK_CMD[@]}"
    echo
} > "$CMD_FILE"
echo "==> kshark topic scan: $TOPICS_DEFAULT"
(cd "$TMP_DIR" && "${KSHARK_CMD[@]}") || true
